...
export const ExplorerCtrl = {
  ...
  async getWallets(params) {
    ...
    if (explorerRecommendedWalletIds === 'NONE' || explorerExcludedWalletIds === 'ALL' && !CoreUtil.isArray(explorerRecommendedWalletIds)) {
      return state.wallets;
    }

    // Fetch only user recommended wallets if the rest is excluded
    if (explorerExcludedWalletIds === 'ALL' && CoreUtil.isArray(explorerRecommendedWalletIds)) {
    ...
    const {
      listings,
      total
    } = await ExplorerUtil.getListings(extendedParams);
